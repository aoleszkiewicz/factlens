...
Model oceniono pięcioma metrykami (Tabela~\ref{tab:metrics}). Wszystkie utrzymują się
...
nieliniowość sieci nie daje istotnej przewagi. Wartością tego modelu jest nie sam
wzrost metryk, lecz kompletna, zweryfikowana pętla treningowa w PyTorchu oraz
rzetelna eliminacja wycieku danych.
...
co sugeruje, że dokładanie danych przyniosłoby co najwyżej marginalną poprawę na tym
zbiorze.
...
(\texttt{said}, \texttt{on wednesday}, \texttt{president donald}), a stronę Fake
...
wycieku nie pojawia się na liście, więc model uczy się stylu i tematu, a nie stempla
redakcyjnego.
