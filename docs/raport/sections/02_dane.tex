...
\begin{itemize}[noitemsep]
  \item \textbf{Duplikaty}: ok. 13{,}9\% artykułów to dokładne duplikaty tekstu,
        ...
        1{,}1\% Real). Brak konfliktów etykiet między klasami, zatem deduplikacja jest bezpieczna.
...
silnie prawoskośny; 95.~percentyl to ok. \textbf{900 słów}, co bywa istotne przy
ewentualnym rozszerzeniu o model sekwencyjny (kandydat na \texttt{max\_seq\_len}).
...
Wszystkie trzy kolumny są więc odrzucane, a model korzysta wyłącznie z \texttt{text}.

\subsection{Czyszczenie tekstu}

Funkcja \texttt{clean\_text} (\texttt{src/data/cleaning.py}) stosuje uporządkowaną
kaskadę wyrażeń regularnych, w której każdy wzorzec ma uzasadnienie w analizie MI powyżej.
